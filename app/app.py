...
def main() :
    ...
    def identite_client2(data, id):
        dt = identite_client(data, id)
        cin = dt["NUM_CIN"]
        return cin
    ...
    def load_credit_population(sample):
        ...
        df_credit = df_credit.loc[df_credit['AMT_CREDIT'] < 200000, :]
        return df_credit
    ...
    <p style="font-size: 20px; font-weight: bold; text-align:center">Département de recouvrement et contentieux EQDOM</p>
    """
    ...
    #Loading CIN selectbox
    cin_id = st.sidebar.selectbox("Cin ID", NUM_CIN)
    ...
    #Average income
    st.sidebar.markdown("<u>Average income (MAD) :</u>", unsafe_allow_html=True)
    st.sidebar.text(rev_moy)

    #AMT CREDIT
    st.sidebar.markdown("<u>Average loan amount (MAD) :</u>", unsafe_allow_html=True)
    ...
    if st.checkbox("Show customer information ?"):
        ...
        #Credit distribution plot
        #data_income = data["AMT_CREDIT"]
        ...
        ax.set(title='Customer Amount Credit', xlabel='Amount Credit', ylabel='')
        st.pyplot(fig)
        ...
        st.subheader("*Credit*")
        st.write("Product/Type : ", infos_client["CSP/PRD"].values[0])
        st.write("Monthly payments : {:.0f}".format(infos_client["AMT_ANNUITY"].values[0]))
        st.write("Application amount  : {:.0f}".format(infos_client["AMT_GOODS_PRICE"].values[0]))
        st.write("Credit amount : {:.0f}".format(infos_client["AMT_CREDIT"].values[0]))
    else:
    ...
    df = pd.DataFrame()        
    for i in data.index:
        t = identite_client2(data, chk_id)
        if str(data["NUM_CIN"][i]) == str(t[chk_id]):    
            df = df._append(data.loc[[i]])
    st.write(df) 
    ...
    if chk_voisins:
        knn = load_knn(sample)
        st.markdown("<u>List of the 10 files closest tidentite_cliento this Customer :</u>", unsafe_allow_html=True)
